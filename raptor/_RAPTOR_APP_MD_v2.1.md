...
- **v1.3:** 기획 API 입출력(I/O) 계약에 MAIN v2.2 JSON 스키마 반영.
- **v2.1 (Current):**
  ...
  - **추가:** 3차 리뷰 식별 버그 (manualAdditions 누락) 픽스 사항 기록
...
| 항목 | v1.3 동작 | v2.1 동작 |
|------|-----------|-----------|
| FFmpeg 런타임 탐지 | 바이너리 자동 탐지 및 런타임 fallback | FFmpeg 전제, 미설치 시 500 반환 |
| 영상 포맷 | MP4·WebM·MOV 3종 자동 변환 | MP4/H264 단일 포맷 |
| duration 처리 | 영상 부족 시 자동 배속 조절 | 고정값(`3|5|7s`), 미달 시 에러 검증 |
| 오버런 처리 | 속도 조절 믹스 | `-t` 플래그를 통한 앞 N초 Trim |
| ngrok 터널링 | 실패 시 즉각 동기 fallback | 터널링 실패 시 500 에러 명시적 반환 |
...
- 만료: `created_at + timedelta(days=14)` (Pydantic 모델 레벨 강제).
